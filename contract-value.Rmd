...
combined_data <- bballref18 %>% 
  ...
  mutate(bbr_legacy_fpts = bbr_pts + (bbr_dreb*1.5) + (bbr_orb*2) + (bbr_ast*2) + (bbr_stl*2.5) + (bbr_blk*2.5) +
         (bbr_tov*-1) + bbr_threes + (bbr_fga*-0.5) + (bbr_fgm*0.5) + bbr_ftm + (bbr_fta*-1)) %>%
  mutate(bbr_franchise_fpts = bbr_pts + (bbr_dreb*1.5) + (bbr_orb*2) + (bbr_ast*2) + (bbr_stl*2.5) + (bbr_blk*2.5) +
         (bbr_tov*-1) + bbr_threes + (bbr_fga*-0.5) + (bbr_fgm*0.5) + bbr_ftm + (bbr_fta*-1)) %>% 
  ...
  mutate(bbr_botb_fpts = bbr_pts + bbr_dreb + (bbr_orb*1.25) + (bbr_ast*1.5) + (bbr_stl*1.5) + (bbr_blk*2) +
         (bbr_tov*-1)+bbr_threes+(bbr_fga*-0.5)+(bbr_fgm*0.5)+(bbr_ftm*0.5)+(bbr_fta*-0.75)) %>% 
  select(player, age, bbr_legacy_fpts, bbr_franchise_fpts, bbr_gsd2_fpts, bbr_botb_fpts, bbr_gms, bbr_mpg,
...
hashalt19 <- hashalt19 %>% 
  ...
  mutate(hta_legacy_fpts = hta_pts + (hta_dreb*1.5) + (hta_orb*2) + (hta_ast*2) + (hta_stl*2.5) + (hta_blk*2.5) +
         (hta_tov*-1) + hta_threes + (hta_fga*-0.5) + (hta_fgm*0.5) + hta_ftm + (hta_fta*-1)) %>%
  mutate(hta_franchise_fpts = hta_pts + (hta_dreb*1.5) + (hta_orb*2) + (hta_ast*2) + (hta_stl*2.5) + (hta_blk*2.5) +
         (hta_tov*-1) + hta_threes + (hta_fga*-0.5) + (hta_fgm*0.5) + hta_ftm + (hta_fta*-1)) %>% 
  ...
  mutate(hta_botb_fpts = hta_pts + hta_dreb + (hta_orb*1.25) + (hta_ast*1.5) + (hta_stl*1.5) + (hta_blk*2) +
         (hta_tov*-1)+hta_threes+(hta_fga*-0.5)+(hta_fgm*0.5)+(hta_ftm*0.5)+(hta_fta*-0.75)) %>% 
  select(player, hta_legacy_fpts, hta_franchise_fpts, hta_gsd2_fpts, hta_botb_fpts, hta_gms, hta_mpg, 
...
hastag19 <- gs_url("https://docs.google.com/spreadsheets/d/13EnBmUWDmxJ86xFzCpaHUuXGW0KADW_bDUVLf0dWFgA/edit#gid=665288709") %>% gs_read(ws = "2019_hashtag_proj") 

hastag19$player <- hastag19$player %>% 
  str_replace_all(c('Á' = 'A', 'ā' = 'a', 'á' = 'a', 'č' = 'c', 'ć' = 'c', 'è' = 'e', 'é' = 'e', 'ê' = 'e',
                    'ö' = 'o', 'ó' = 'o', 'Š' = 'S', 'š' = 's', 'ū' = 'u', 'ý' = 'y', 'Ž' = 'Z', 'ž' = 'z',
                    'P.J.' = 'PJ','C.J.' = 'CJ', 'R.J.' = 'RJ', 'T.J.' = 'TJ', 'D.J.' = 'DJ', 'J.J.' = 'JJ',
                    'Louis' = 'Lou', 'Edrice' = 'Bam', 'Waller-Prince' = 'Prince', "O.G." = "OG", "Demarre" = "DeMarre",
...
hastag19 <- hastag19 %>% 
  ...
  mutate(ht_legacy_fpts = ht_pts + (ht_dreb*1.5) + (ht_orb*2) + (ht_ast*2) + (ht_stl*2.5) + (ht_blk*2.5) +
         (ht_tov*-1) + ht_threes + (ht_fga*-0.5) + (ht_fgm*0.5) + ht_ftm + (ht_fta*-1)) %>%
  mutate(ht_franchise_fpts = ht_pts + (ht_dreb*1.5) + (ht_orb*2) + (ht_ast*2) + (ht_stl*2.5) + (ht_blk*2.5) +
         (ht_tov*-1) + ht_threes + (ht_fga*-0.5) + (ht_fgm*0.5) + ht_ftm + (ht_fta*-1)) %>% 
  ...
  mutate(ht_botb_fpts = ht_pts + ht_dreb + (ht_orb*1.25) + (ht_ast*1.5) + (ht_stl*1.5) + (ht_blk*2) +
         (ht_tov*-1)+ht_threes+(ht_fga*-0.5)+(ht_fgm*0.5)+(ht_ftm*0.5)+(ht_fta*-0.75)) %>% 
  select(player, ht_legacy_fpts, ht_franchise_fpts, ht_gsd2_fpts, ht_botb_fpts, ht_gms, ht_mpg, 
         ht_pts, ht_dreb, ht_orb, ht_ast, ht_stl, ht_blk, ht_tov, ht_threes, ht_fga, ht_fgm, ht_fgp, ht_ftm, ht_fta, ht_ftp)

combined_data <- combined_data %>% 
  full_join(hastag19, by = "player")
...
select_data <- combined_data %>% 
  select(player, age,
         bbr_legacy_fpts, bbr_franchise_fpts, bbr_gsd2_fpts, bbr_botb_fpts, bbr_gms, bbr_mpg,
         hta_legacy_fpts, hta_franchise_fpts, hta_gsd2_fpts, hta_botb_fpts, hta_gms, hta_mpg,
         ht_legacy_fpts, ht_franchise_fpts, ht_gsd2_fpts, ht_botb_fpts, ht_gms, ht_mpg, 
         spottrac_2019,	spottrac_2020, spottrac_2021, spottrac_2022, spottrac_2023, spottrac_rfa,
...
select_data %>% 
  mutate(bbr_botb_value_19 = bbr_botb_fpts / spottrac_2019) %>% 
  mutate(bbr_botb_value_20 = bbr_botb_fpts / spottrac_2020) %>%
  mutate(bbr_botb_value_21 = bbr_botb_fpts / spottrac_2021) %>%
  #filter(bbr_gms > 50) %>% 
  #filter(bbr_botb_fpts > 30) %>% 
  #filter(age < 25) %>% 
  arrange(-bbr_botb_value_19) %>% 
  select(player, age, bbr_botb_fpts, bbr_botb_value_19, bbr_botb_value_20, bbr_botb_value_21, spottrac_2019, spottrac_2020, spottrac_2021, spottrac_rfa)
```

```{r hashtag_alt evaluations in botb}
select_data %>% 
  mutate(hta_botb_value_19 = hta_botb_fpts / spottrac_2019) %>% 
  mutate(hta_botb_value_20 = hta_botb_fpts / spottrac_2020) %>%
  mutate(hta_botb_value_21 = hta_botb_fpts / spottrac_2021) %>%
  #filter(hta_gms > 50) %>% 
  #filter(hta_botb_fpts > 30) %>% 
  #filter(age < 25) %>% 
  arrange(-hta_botb_value_19) %>% 
  select(player, age, hta_botb_fpts, hta_botb_value_19, hta_botb_value_20, hta_botb_value_21, spottrac_2019, spottrac_2020, spottrac_2021, spottrac_rfa)
```

```{r hashtag evaluations in botb}
select_data %>% 
  mutate(ht_botb_value_19 = ht_botb_fpts / spottrac_2019) %>% 
  mutate(ht_botb_value_20 = ht_botb_fpts / spottrac_2020) %>%
  mutate(ht_botb_value_21 = ht_botb_fpts / spottrac_2021) %>%
  #filter(ht_gms > 50) %>% 
  #filter(ht_botb_fpts > 30) %>% 
  #filter(age < 25) %>% 
  arrange(-ht_botb_value_19) %>% 
  select(player, age, ht_botb_fpts, ht_botb_value_19, ht_botb_value_20, ht_botb_value_21, spottrac_2019, spottrac_2020, spottrac_2021, spottrac_rfa)
```
...
select_data %>% 
  ...
  #filter(bbr_ld_fpts > 30) %>% 
  #filter(age < 25) %>% 
  arrange(-bbr_ld_value_19) %>% 
  select(player, age, bbr_ld_fpts, bbr_ld_value_19, bbr_ld_value_20, bbr_ld_value_21, ld_2019, ld_2020, ld_2021, ld_rfa, ld_gleague, ld_intl)
```
...
select_data %>% 
  ...
  #filter(hta_ld_fpts > 30) %>% 
  #filter(age < 25) %>% 
  arrange(-hta_ld_value_19) %>% 
  select(player, age, hta_ld_fpts, hta_ld_value_19, hta_ld_value_20, hta_ld_value_21, ld_2019, ld_2020, ld_2021, ld_rfa, ld_gleague, ld_intl)
```
...
select_data %>% 
  ...
  select(player, age, ht_ld_fpts, ht_ld_value_19, ht_ld_value_20, ht_ld_value_21, ld_2019, ld_2020, ld_2021, ld_rfa, ld_gleague, ld_intl)
```
