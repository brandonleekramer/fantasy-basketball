...
labs <- c("points(halved)","rebounds","assists","steals","blocks","turnovers","threes","fg%","ft%")
scores <- as.list(as.data.frame(t(inv_ht_matrix)))
...
inv_ht_matrix <- inv_projections %>% 
  select(inv_abbreviated, ht_team_pts:ht_team_ftp) %>% 
  filter(inv_abbreviated == "MLDR" | inv_abbreviated == "ALi" ) %>% 
  mutate(ht_team_pts = ht_team_pts/2, 
         ht_team_stls = ht_team_stls*2,
         ht_team_blks = ht_team_blks*2,
         ht_team_threes = ht_team_threes*2,
         ht_team_fgp = ht_team_fgp*100,
         ht_team_ftp = ht_team_ftp*100) %>% 
  mutate_if(is.numeric, round, digits = 1) %>% 
  rename(points = ht_team_pts, rebounds = ht_team_rebs, assists = ht_team_asts, steals = ht_team_stls,
...
inv_ht_matrix <- rbind(rep(100,10) , rep(0,10) , inv_ht_matrix)

#colors_border=c( rgb(0.2,0.5,0.5,0.9), rgb(0.8,0.2,0.5,0.9)  )
#colors_in=c( rgb(0.2,0.5,0.5,0.4), rgb(0.8,0.2,0.5,0.4)  )

coul <- as.character(c("#a31a2d","#255e58"))
colors_border <- coul
library(scales)
colors_in <- alpha(coul,0.3)

# plot with default options:
radarchart(inv_ht_matrix, axistype=0 , 
    #custom polygon
    pcol=colors_border , pfcol=colors_in , plwd=2.5 , plty=1,
    #custom the grid
    cglcol="grey", cglty=1.2, axislabcol="black", caxislabels=seq(0,100,20), cglwd=0.8,
    #custom labels
    vlcex=0.8 
    )
legend(x=0.85, y=1.25, legend = rownames(inv_ht_matrix[-c(1,2),]), bty = "n", pch=20 , col=colors_in , text.col = "black", cex=0.8, pt.cex=2.8)
