...
botb_inseason <- botb_inseason %>% 
  rename(
         #trade_comp = "Trade",
         fntx_gms = "GP", fntx_fppg = "FP/G", botb_team = "Status", position = "Position") %>%
  rename(nba_team = "Team") %>% 
  mutate(nba_team = recode(nba_team, PHO = "PHX", SA = "SAS", NY = "NYK", NO = "NOP", GS = "GSW")) %>% 
  select(player, fntx_fppg, fntx_gms, botb_team, nba_team, position, 
         #trade_comp
         )
...
select_data <- combined_data %>% 
  ...
  select(player, age, fntx_fppg, bbr_botb_fpts, hta_botb_fpts, ht_botb_fpts, 
         ...
         botb_rfa, botb_gleague, nba_team, position 
...
rm(bballref18, hashtag19, hashalt19, franchise_contracts, botb_contracts, legacy_contracts, botb_contracts)
```
