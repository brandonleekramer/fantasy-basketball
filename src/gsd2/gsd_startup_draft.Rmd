...
selected_players <- read_csv("~/Documents/fantasy-basketball/data/nba_fantasy_data/gsd2_player_rankings - Sheet1.csv") %>% mutate(player = tolower(player)) %>% clean_nba_names()
...
gsd2_data_contracts  %>% 
  ...
  mutate(player = tolower(player)) %>% 
  filter(!grepl(selected_players, player) & 
           y2020_21 < 20000000 & gsd2_fppg_bbr > 22) 
```
