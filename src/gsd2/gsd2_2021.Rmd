...
gsd2_inseason <- gsd2_inseason %>% 
  rename(fntx_pts = "PTS", fntx_orb = "OREB", fntx_dreb = "DREB", 
         ...
         fntx_ftm = "FTM", fntx_gms = "GP", fntx_fppg = "FP/G",
         trade_comp = "Trade", 
...
select_data <- combined_data  %>% 
   mutate(fr_2020 = replace_na(fr_2020, 0),
          fr_2020 = ifelse(fr_2020 == 0, fr_2019, fr_2020))

# Best FA available 
select_data %>% filter(fr_team == "FA")
```
...
select_data %>% 
  ...
  select(player, fntx_fppg, fr_fppg_ht, fr_2020, fr_2021, fr_rfa, 
          fr_injured, fr_gleague, fr_intl)
```
...
bad_teams <- c("ORL", "TOR", "MIA", "SAS", "PHX", "DET", "SAC", "CHI", "BKN", "UTA")

select_data %>% 
  mutate(fntx_value_19 = fr_fppg_ht / fr_2019 * 10000) %>% 
  mutate(fntx_value_20 = fr_fppg_ht / fr_2020 * 10000) %>%
  mutate(fntx_value_21 = fr_fppg_ht / fr_2021 * 10000) %>%
  #mutate(diff = fntx_fppg - fr_fppg_ht) %>%
  filter(fr_fppg_ht > 22) %>% 
  #filter(fr_2019 < 17000000) %>% 
  filter(fr_team %in% bad_teams) %>%
  #filter(fr_team == "CLE") %>% 
  #filter(age < 25) %>% 
  #filter(grepl("SF", position)) %>% 
  arrange(-fntx_value_20) %>% 
  #arrange(-fntx_value_19) %>% 
  select(player, fntx_fppg, fr_2019, fr_2020, fr_2021, fr_rfa, 
         fr_fppg_ht, fr_injured, fr_gleague, fr_intl)
```
...
temp_1 <- select_data %>% 
  ...
  #filter(fr_gleague == "0") %>% 
  #filter(fr_intl == "0") %>%  
  filter(fr_team != "FA" & fr_team != "W (Sat)" & fr_team != "W (Fri)" & fr_team != "W (Wed)") %>% 
  drop_na(fr_team) %>%
  group_by(fr_team) %>% top_n(15, wt = fntx_fppg) %>% 
  summarize(avg_fppg = mean(fntx_fppg, na.rm = TRUE)) 
temp_2 <- select_data %>% 
  #filter(fr_injured == "0") %>% 
  #filter(fr_gleague == "0") %>% 
  #filter(fr_intl == "0") %>% 
  ...
  drop_na(fr_team) %>%
  group_by(fr_team) %>% top_n(15, wt = fntx_fppg) %>% 
  summarize(fr_players = n()) %>% filter(fr_team != "NA") %>% 
...
temp_1 <- select_data %>% 
  ...
  group_by(fr_team) %>% top_n(15, wt = fr_fppg_ht) %>% 
  summarize(avg_fppg = mean(fr_fppg_ht, na.rm = TRUE)) 
temp_2 <- select_data %>% 
  #filter(fr_injured == "0") %>% 
  #filter(fr_gleague == "0") %>% 
  #filter(fr_intl == "0") %>% 
...
temp_1 <- select_data %>% 
  ...
  #filter(fr_intl == "0") %>%  
  filter(fr_team != "FA") %>% drop_na(fr_team) %>%
  group_by(trade_comp) %>% top_n(15, wt = fr_fppg_ht) %>% 
  summarize(avg_fppg = mean(fr_fppg_ht, na.rm = TRUE)) 
temp_2 <- select_data %>% 
  #filter(fr_injured == "0") %>% 
  #filter(fr_gleague == "0") %>% 
  ...
  filter(fr_team != "FA") %>% drop_na(fr_team) %>%
  group_by(trade_comp) %>% top_n(15, wt = fr_fppg_ht) %>% 
  summarize(fr_players = n()) %>% filter(trade_comp != "NA") %>% 
  full_join(temp_1, temp_2, by = "trade_comp")
temp_3 <- select_data %>% 
  #filter(fr_injured == "0") %>% 
 # filter(fr_gleague == "0") %>% 
