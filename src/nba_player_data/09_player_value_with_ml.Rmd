...
nba_data <- nba_data_raw %>% 
  ...
  arrange(-fppg) %>% 
  drop_na(salary, fppg)
nba_data %>% select(year, position, player, age, team, fppg, salary, mins)
```
...
```{r}
model1_baseline <- lm(gsd2_fppg_bbr ~ salary, data=nba_data) # fit the model
...
That baseline model shows that salary alone does a pretty terrible job of explaining production. Here, the adjusted R-squared is only 0.2261. 

```{r, fig.width=10, fig.height=5}
nba_data <- arrange(nba_data, desc(residuals_m1)) %>%
  mutate(rank = 1:nrow(nba_data)) %>% 
  select(rank, player, position, age, gsd2_fppg_bbr, residuals_m1, games, everything())

ggplot(nba_data, aes(x = salary, y = gsd2_fppg_bbr)) +
...
```{r}
model2_baseline <- lm(gsd2_fppg_bbr ~ salary + year, data=nba_data) # fit the model
...
nba_data <- arrange(nba_data, desc(residuals_m2)) %>%
  mutate(rank = 1:nrow(nba_data)) %>% 
  select(rank, player, position, age, gsd2_fppg_bbr, residuals_m2, games, everything())

nba_data %>% 
  select(player, year, gsd2_fppg_bbr, salary, residuals_m1, residuals_m2)
```

Let's add age, minutes, games, and positions

```{r}
model3_baseline <- lm(gsd2_fppg_bbr ~ salary + year + mins + games + age +
...
nba_data %>% 
  select(player, year, gsd2_fppg_bbr, salary, residuals_m1, residuals_m2, residuals_m3)
```

```{r}
model4_baseline <- lm(gsd2_fppg_bbr ~ salary + year + mins + games + 
                      pts + orb + drb + ast + stl + blk + ft_percent + fg_percent +
                      pos_pg + pos_sg + pos_sf + pos_pf + pos_c, data=nba_data) # fit the model
nba_data$predicted_m4 <- predict(model4_baseline)   # Save the predicted values
nba_data$residuals_m4 <- residuals(model4_baseline) # Save the residual values
summary(model4_baseline)
```

```{r}
nba_data <- arrange(nba_data, desc(residuals_m4)) %>%
  mutate(rank = 1:nrow(nba_data)) %>% 
  mutate(fppg = round(gsd2_fppg_bbr, 2),
         residuals_m1 = round(residuals_m1, 3),
         residuals_m2 = round(residuals_m2, 3),
         residuals_m3 = round(residuals_m3, 3),
         residuals_m4 = round(residuals_m4, 3)) %>% 
  select(rank, player, position, age, fppg, 
         residuals_m1, residuals_m2, residuals_m4, games, everything())

nba_data %>% 
  select(player, year, age, fppg, salary, residuals_m1, residuals_m3, residuals_m4)
```
...
ggplot(nba_data, aes(x = salary, y = gsd2_fppg_bbr)) +
  geom_smooth(method = "lm", formula = gsd2_fppg_bbr ~ salary + year, se = FALSE, color = "black") +      
  geom_segment(aes(xend = salary, yend = predicted_m2), alpha = .2) +   
  geom_point(aes(color = abs(residuals_m2), size = abs(residuals_m2))) +  
  scale_color_continuous(low = "#386890", high = "#40E0D0") +     
  guides(color = FALSE, size = FALSE) +                             
  geom_point(aes(y = predicted_m2), shape = 1) + 
  scale_x_continuous(breaks = c(0, 5000000, 10000000, 15000000, 20000000, 25000000, 30000000, 35000000),
                     labels = c("0M", "5M", "10M", "15M", "20M", "25M", "30M", "35M")) +
  theme_minimal() + 
  xlab("Fantasy Points Per Game") + ylab("Annual Salary") +
