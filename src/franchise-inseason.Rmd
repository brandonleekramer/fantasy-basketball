...
rm(list = ls())
for (pkg in c("tidyverse", "plotly", "ggrepel", "modelr")) {library(pkg, character.only = TRUE)}
...
franchise_inseason <- read_csv("nba_fantasy_data - franchise_inseason_last30.csv")
...
franchise_inseason <- franchise_inseason %>% 
  rename(fntx_pts = "PTS", fntx_orb = "OREB", fntx_dreb = "DREB", fntx_ast = "AST", fntx_stl = "ST", 
         ...
         trade_comp = "Trade", position = "Position",
  ...
  select(player, position, fntx_fppg, fntx_ld_fpts, fntx_fr_fpts, fntx_gsd2_fpts, fntx_botb_fpts, fntx_gms,  
...
select_data <- combined_data %>% 
  ...
  select(player, age, position, fntx_fppg, bbr_fr_fpts, hta_fr_fpts, ht_fr_fpts, 
  ...
  mutate(playoff1 = week21, playoff2 = week22, playoff3 = week23, playoff4 = week24) %>% 
  ...
  mutate(week17_totals = (fntx_fppg * (week17 + week18))) %>%
  mutate(week18_totals = (fntx_fppg * week19)) %>%
  mutate(week19_totals = (fntx_fppg * week20)) %>%
  # playoff totals 
  ...
  filter(player != "Rodions Kurucs" & player != "Jontay Porter" & 
         ...
         player != "Didi Louzada Silva" &  player != "Alessandro Gentile" &
          player != "Miye Oni" &  player != "Olivier Hanlan" ) 
...
select_data %>% 
  ...
  select(player, age, fntx_fppg, fr_2019, fr_2020, fr_2021, fr_rfa, 
         bbr_fr_fpts, hta_fr_fpts, ht_fr_fpts, fr_injured, fr_gleague, fr_intl, position)
```
...
select_data %>% 
  ...
  filter(fntx_fppg > 28) %>% 
  filter(fr_2019 < 7000000) %>% 
  #filter(fr_team == "MIA" | fr_team == "UTA" | fr_team == "DAL" | fr_team == "LAC" |  
  #       fr_team == "MEM" | fr_team == "WAS" | fr_team == "BKN" | fr_team == "MIL" | 
  #       fr_team == "DET" | fr_team == "IND" | fr_team == "ORL" | fr_team == "OKC"
  #       # | fr_team == "NOP" | fr_team == "CHI"  
  #      ) %>%
  #filter(fr_team == "CLE") %>% 
  #filter(age < 25) %>% 
  #filter(grepl("PF", position)) %>% 
  arrange(-fntx_value_19) %>% 
  select(player, age, position, fr_team, fntx_fppg, ht_fr_fpts, diff, fntx_value_19, fntx_value_20, 
...
weekly_totals_plot <- weekly_totals %>% 
  #filter(week > 12 & week < 20) %>% # games that have passed in regular season 
...
p <- select_data %>% 
  #filter(fr_team == "CLE") %>% 
  filter(fntx_fppg != 0) %>% 
  ggplot(aes(fr_2019, fntx_fppg, label = player)) + 
  geom_point(color = dplyr::case_when(select_data$trade_comp == "CLE" ~ "#ff4f00",
                                      TRUE ~ "#000000")) +
  theme_minimal() +
  geom_text_repel(data = subset(select_data, fr_team == "CLE"),
                  size = 4,
                  box.padding = 0.35, 
                  point.padding = 0.5,
                  force         = 100,
...
lm1 <- lm(fntx_fppg ~ fr_2019, data = select_data)
lm1_resid <- resid(lm1)
qqnorm(lm1_resid) 
qqline(lm1_resid)
...
# value by players 
test_data %>% 
  select(player, trade_comp, resid) %>% 
  drop_na(resid) %>% 
  #arrange(-resid) %>% 
  filter(trade_comp != "FA") %>% 
  group_by(trade_comp) %>% 
  top_n(15) %>% 
  arrange(trade_comp)
...
loess1 <- loess(fntx_fppg ~ fr_2019, data = clean_data, span = 0.10)
loess2 <- loess(fntx_fppg ~ fr_2019, data = clean_data, span = 0.25)
loess3 <- loess(fntx_fppg ~ fr_2019, data = clean_data, span = 0.50)
smoothed10 <- predict(loess1) 
smoothed25 <- predict(loess2) 
smoothed50 <- predict(loess3) 

plot(clean_data$fntx_fppg, x=clean_data$fr_2019, 
     type="l", main="Loess Smoothing and Prediction", 
     xlab="Date", ylab="Unemployment (Median)")
lines(smoothed10, x=clean_data$fr_2019, col="red")
lines(smoothed25, x=clean_data$fr_2019, col="green")
lines(smoothed50, x=clean_data$fr_2019, col="blue")
...
temp_3 <- select_data %>% 
  ...
  summarize(cap_used = sum(fr_2019, na.rm = TRUE)) %>% 
  mutate(cap_left = 109400000 - cap_used)
team_totals <- full_join(temp_2, temp_3, by = "fr_team") %>% 
  mutate(team_total = avg_fppg * fr_players) %>% 
  arrange(-team_total) %>%
  select(fr_team, fr_players, avg_fppg, team_total, cap_used, cap_left); rm(temp_1, temp_2); rm(temp_3)
...
select_data <- select_data %>% 
  filter(fr_team != "FA") %>%
  mutate(countPG = ifelse(test = str_detect(string = position, pattern = "\\b(?i)(PG)\\b"), yes = 1, no = 0)) %>%
  mutate(countSG = ifelse(test = str_detect(string = position, pattern = "\\b(?i)(SG)\\b"), yes = 1, no = 0)) %>%
  mutate(countSF = ifelse(test = str_detect(string = position, pattern = "\\b(?i)(SF)\\b"), yes = 1, no = 0)) %>%
  mutate(countPF = ifelse(test = str_detect(string = position, pattern = "\\b(?i)(PF)\\b"), yes = 1, no = 0)) %>%
  mutate(countC = ifelse(test =  str_detect(string = position, pattern = "\\b(?i)(C)\\b"),  yes = 1, no = 0)) 

position_totals <- select_data %>% 
  filter(countPG == 1) %>% 
  group_by(fr_team) %>% 
  summarise(PGsOnTeam = n_distinct(player)) %>% 
  mutate(prcPG = PGsOnTeam / 15)
position_totals <- select_data %>% 
  filter(countSG == 1) %>% 
  group_by(fr_team) %>% 
  summarise(SGsOnTeam = n_distinct(player)) %>% 
  right_join(position_totals, by = "fr_team") %>%
  mutate(prcSG = SGsOnTeam / 15)
position_totals <- select_data %>% 
  filter(countSF == 1) %>% 
  group_by(fr_team) %>% 
  summarise(SFsOnTeam = n_distinct(player)) %>%
  right_join(position_totals, by = "fr_team") %>%
  mutate(prcSF = SFsOnTeam / 15)
position_totals <- select_data %>% 
  filter(countPF == 1) %>% 
  group_by(fr_team) %>% 
  summarise(PFsOnTeam = n_distinct(player)) %>% 
  right_join(position_totals, by = "fr_team") %>%
  mutate(prcPF = PFsOnTeam / 15)
position_totals <- select_data %>% 
  filter(countC == 1) %>% 
  group_by(fr_team) %>% 
  summarise(CsOnTeam = n_distinct(player)) %>% 
  right_join(position_totals, by = "fr_team") %>%
  mutate(prcC = CsOnTeam / 15) %>% 
  replace(., is.na(.), 0)
...
contracts_by_position <- select_data %>% 
  filter(countPG == 1) %>% 
  drop_na(fr_2019) %>% 
  group_by(fr_team) %>% 
  summarize(CostForPGs = sum(fr_2019)) %>%
  mutate(CostPerPG = CostForPGs / n_distinct(player))

contracts_by_position <- select_data %>% 
  filter(countSG == 1) %>% 
  drop_na(fr_2019) %>% 
  group_by(fr_team) %>% 
  summarize(CostForSGs = sum(fr_2019)) %>% 
  right_join(contracts_by_position, by = "fr_team") 
contracts_by_position <- select_data %>% 
  filter(countSF == 1) %>% 
  drop_na(fr_2019) %>% 
  group_by(fr_team) %>% 
  summarize(CostForSFs = sum(fr_2019)) %>% 
  right_join(contracts_by_position, by = "fr_team") 
contracts_by_position <- select_data %>% 
  filter(countPF == 1) %>% 
  drop_na(fr_2019) %>% 
  group_by(fr_team) %>% 
  summarize(CostForPFs = sum(fr_2019)) %>% 
  right_join(contracts_by_position, by = "fr_team") 
contracts_by_position <- select_data %>% 
  filter(countC == 1) %>% 
  drop_na(fr_2019) %>% 
  group_by(fr_team) %>% 
  summarize(CostForCs = sum(fr_2019)) %>% 
  right_join(contracts_by_position, by = "fr_team") %>% 
  replace(., is.na(.), 0)
...
position_model <- lm(team_total ~ prcPG + prcSG + prcSF + prcPF + prcC, data = team_totals)
