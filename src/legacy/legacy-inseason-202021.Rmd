...
legacy_inseason <- read_csv("Fantrax-Players-Legacy Dynasty 2019_2020.csv")
...
combined_data <- hashtag20 %>% 
  ...
  arrange(-fr_fppg_ht) %>% 
  select(player, fr_fppg_ht, FPts, fntx_fppg, everything(), 
         -ld_fppg_ht, -botb_fppg_ht, -gsd2_fppg_ht, -ld_position)
 
select_data <- combined_data  %>% 
   mutate(ld_2020 = replace_na(ld_2020, 0),
          ld_2020 = ifelse(ld_2020 == 0, ld_2019, ld_2020))
...
select_data %>% 
  ...
  arrange(-fr_fppg_ht) %>%         # arrange by ht projections
  select(player, fr_fppg_ht, fntx_fppg, ld_2020, ld_2021, ld_2022, ld_rfa, ld_gleague, ld_intl)
```
...
pre_trade_fntx <- pre_trade_calc(df = select_data, league = "legacy",
                                 ...
                                 fppg_col = fntx_fppg, cap_col = ld_2020)
...
prediction_data <- combined_data %>% 
  ...
  filter(ld_team != "FA") %>% 
  select(player, position,fr_fppg_ht, fntx_fppg, ld_team,ld_2020)

fit_fntx <- lm(fntx_fppg ~ ld_2020, data=prediction_data) # fit the model
prediction_data$fntx_predicted <- predict(fit_fntx)   # Save the predicted values
prediction_data$fntx_residuals <- residuals(fit_fntx) # Save the residual values

fit_ht <- lm(fr_fppg_ht ~ ld_2020, data=prediction_data) # fit the model
prediction_data$ht_predicted <- predict(fit_ht)   # Save the predicted values
prediction_data$ht_residuals <- residuals(fit_ht) # Save the residual values

prediction_data <- prediction_data %>%
  mutate(rank = 1:nrow(prediction_data)) %>% 
  select(player, fr_fppg_ht, fntx_fppg, ht_residuals, fntx_residuals, 
         position, ld_team, ld_2020, ht_residuals, fntx_predicted, everything()) %>% 
  arrange(-ht_residuals)

# fntx residuals 
ggplot(prediction_data, aes(x = ld_2020, y = fntx_fppg)) +
  geom_smooth(method = "lm", se = FALSE, color = "black") +      
  geom_segment(aes(xend = ld_2020, yend = fntx_predicted), alpha = .2) +   
  geom_point(aes(color = abs(fntx_residuals), size = abs(fntx_residuals))) +  
  scale_color_continuous(low = "#386890", high = "orange") +     
  guides(color = FALSE, size = FALSE) +                             
  geom_point(aes(y = fntx_predicted), shape = 1) +
  theme_minimal()

# ht residuals 
ggplot(prediction_data, aes(x = ld_2020, y = fr_fppg_ht)) +
  geom_smooth(method = "lm", se = FALSE, color = "black") +      
  geom_segment(aes(xend = ld_2020, yend = ht_predicted), alpha = .2) +   
  geom_point(aes(color = abs(ht_residuals), size = abs(ht_residuals))) +  
  scale_color_continuous(low = "#386890", high = "orange") +     
  guides(color = FALSE, size = FALSE) +                             
  geom_point(aes(y = ht_predicted), shape = 1) +
  theme_minimal()

hist(prediction_data$fntx_residuals)
```
```{r}
prediction_data %>% 
  filter(ld_team == "NOP")
