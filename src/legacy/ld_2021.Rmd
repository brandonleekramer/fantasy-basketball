...
legacy_contracts <- read_csv("nba_fantasy_data - ld_contracts_202021.csv")
  
legacy_inseason <- legacy_inseason %>% mutate(Trade = Status)
...
legacy_contracts <- legacy_contracts %>% 
  clean_names() %>% 
  clean_nba_names(player) %>% 
  mutate(ld_2020 = as.numeric(ld_2020), ld_2021 = as.numeric(ld_2021),
         ld_2022 = as.numeric(ld_2022), ld_2023 = as.numeric(ld_2023))
...
combined_data <- hashtag20 %>% 
  ...
  select(player, fr_fppg_ht, FPts, fntx_fppg, everything(), 
         -ld_fppg_ht, -botb_fppg_ht, -gsd2_fppg_ht)
 
select_data <- combined_data 

select_data %>% filter(ld_team == "FA") %>% arrange(-fntx_fppg)
```
...
select_data %>% 
  ...
  select(player, fr_fppg_ht, fntx_fppg, ld_2020, ld_2021, ld_2022, gleague2021)
```

# Value Calculators 

```{r hashtag evaluations in fr}
bad_teams <- c("CLV", "DAL", "HOU", "BKN", "BOS", "SAC", "MEM", "ORL")

select_data %>% 
  mutate(raw_val_20 = fntx_fppg / ld_2020 * 10000) %>%
  mutate(raw_val_21 = fntx_fppg / ld_2021 * 10000) %>%
  mutate(raw_val_20 = round(raw_val_20, 3),
         raw_val_21 = round(raw_val_21, 3)) %>% 
  #mutate(diff = fntx_fppg - fr_fppg_ht) %>%
  filter(fr_fppg_ht > 24) %>% 
  #filter(fr_2019 < 17000000) %>% 
  #filter(ld_2019 %in% bad_teams) %>%
  #filter(fr_team == "CLE") %>% 
  #filter(age < 25) %>% 
  #filter(grepl("SF", position)) %>% 
  arrange(-raw_val_20) %>% 
  #arrange(-fntx_value_19) %>% 
  select(player, raw_val_20, fntx_fppg, fr_fppg_ht, ld_2020, ld_2021)
```
...
prediction_data %>% 
  ...
  mutate(cost_cats = recode(cost_cats,`0`="UFA",`1`="$<5M",`2`="$5-$9.9M",`3`="$10-14.9M",
