...
fit <- lm(ld_fppg_bbr ~ fr_2019, data=test_data) # fit the model
test_data$predicted <- predict(fit)   # Save the predicted values
test_data$residuals <- residuals(fit) # Save the residual values
ggplot(test_data, aes(x = fr_2019, y = ld_fppg_bbr)) +
