...
select_data %>% 
  ...
  select(player, fr_fppg_ht, fntx_fppg, fr_2019, fr_2020, fr_2021, fr_rfa, 
...
bad_teams <- c("MIL", "MIA", "POR", "CHA", "PHI", "DET", "ORL", "IND", "MEM")
...
select_data %>% 
  ...
  select(player, fr_team, fntx_fppg, fr_fppg_ht, fr_2020, fr_2021, fr_rfa, 
          fr_injured, fr_gleague, fr_intl)
```
...
```
```{r}
bad_teams <- c("MIL", "MIA", "POR", "CHA", "PHI", "DET", "ORL", "IND", "MEM", "NOP")

prediction_data %>% 
  filter(teams %in% bad_teams) %>% 
  select(player, teams, salaries, everything())
```
