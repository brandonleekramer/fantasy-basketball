...
franchise_inseason <- franchise_inseason %>% 
  rename(fntx_pts = "PTS", fntx_orb = "OREB", fntx_dreb = "DREB", fntx_ast = "AST", fntx_stl = "ST", 
         ...
         fntx_ftm = "FTM", fntx_fta = "FTA", fntx_gms = "GP", fntx_fppg = "FP/G", fr_team = "Status", trade_comp = "Trade") %>%
  ...
  select(player, fntx_fppg, fntx_ld_fpts, fntx_fr_fpts, fntx_gsd2_fpts, fntx_botb_fpts, fntx_gms,  
         ...
         fntx_tov, fntx_threes, fntx_fga, fntx_fgm, fntx_ftm, fntx_fta, fr_team, trade_comp)

select_data <- combined_data %>% 
  full_join(franchise_inseason, by = "player") %>% 
  select(player, age, fntx_fppg, 
         ...
         fr_rfa, fr_injured, fr_gleague, fr_intl, trade_comp, fr_position)
...
select_data %>% 
  ...
  select(player, age, fntx_fppg, fr_2019, fr_2020, fr_2021, fr_rfa, bbr_fr_fpts, hta_fr_fpts, ht_fr_fpts, fr_injured, fr_gleague, fr_intl)
```
...
select_data %>% 
  ...
  #filter(bbr_gms > 50) %>%
  filter(fr_position == "3/3" | fr_position == "2/3" | fr_position == "3/4") %>% 
  ...
  select(player, fr_team, age, bbr_fr_fpts, bbr_fr_value_19, bbr_fr_value_20, #bbr_fr_value_21, 
...
select_data %>% 
  ...
  filter(fr_team == "MIA" | fr_team == "UTA" | fr_team == "DAL" | fr_team == "LAC" |  
         fr_team == "MEM" | fr_team == "WAS" | fr_team == "BKN" | fr_team == "MIL" | fr_team == "DET" |
         fr_team == "IND" | #fr_team == "NOP" | fr_team == "CHI" | 
           fr_team == "ORL" ) %>%
  #filter(fr_team == "CLE") %>% 
  #filter(age < 25) %>% 
...
select_data %>% group_by(fr_team) %>% count(fr_team)
...
temp_1 <- select_data %>% 
  ...
  #filter(fr_intl == "0") %>%  
  filter(fr_team != "FA") %>% drop_na(fr_team) %>%
  group_by(fr_team) %>% summarize(avg_fppg = mean(fntx_fppg, na.rm = TRUE)) 
temp_2 <- select_data %>% 
  #filter(fr_injured == "0") %>% 
  #filter(fr_gleague == "0") %>% 
  #filter(fr_intl == "0") %>% 
...
temp_1 <- select_data %>% 
  ...
  #filter(fr_intl == "0") %>%  
  filter(fr_team != "FA") %>% drop_na(fr_team) %>%
  group_by(trade_comp) %>% summarize(avg_fppg = mean(fntx_fppg, na.rm = TRUE)) 
temp_2 <- select_data %>% 
  #filter(fr_injured == "0") %>% 
  #filter(fr_gleague == "0") %>% 
  #filter(fr_intl == "0") %>% 
