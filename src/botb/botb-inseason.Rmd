---
title: "BOTB In-Season Fantasy Tools"
...
setwd("C:/Users/bkram/CloudStation/NBA/fantasy-basketball/data")
...
botb_inseason <- read_csv("nba_fantasy_data - botb_inseason.csv")
#botb_inseason <- read_csv("nba_fantasy_data - botb_inseason_last30.csv")
...
combined_data <- bballref18 %>% 
  ...
  mutate(bbr_botb_fpts = bbr_pts + (bbr_dreb*1.5) + (bbr_orb*2) + (bbr_ast*2) + (bbr_stl*2.5) + (bbr_blk*2.5) +
  ...
  mutate(bbr_botb_fpts = bbr_pts + bbr_dreb + (bbr_orb*1.25) + (bbr_ast*1.5) + (bbr_stl*1.5) + (bbr_blk*2) +
         (bbr_tov*-1)+bbr_threes+(bbr_fga*-0.5)+(bbr_fgm*0.5)+(bbr_ftm*0.5)+(bbr_fta*-0.75)) %>% 
  select(player, age, bbr_botb_fpts, bbr_botb_fpts, bbr_botb_fpts, bbr_botb_fpts, bbr_gms, bbr_mpg,
...
hashalt19 <- hashalt19 %>% 
  ...
  mutate(hta_botb_fpts = hta_pts + (hta_dreb*1.5) + (hta_orb*2) + (hta_ast*2) + (hta_stl*2.5) + (hta_blk*2.5) +
  ...
  mutate(hta_botb_fpts = hta_pts + hta_dreb + (hta_orb*1.25) + (hta_ast*1.5) + (hta_stl*1.5) + (hta_blk*2) +
         (hta_tov*-1)+hta_threes+(hta_fga*-0.5)+(hta_fgm*0.5)+(hta_ftm*0.5)+(hta_fta*-0.75)) %>% 
  select(player, hta_botb_fpts, hta_botb_fpts, hta_botb_fpts, hta_botb_fpts, hta_gms, hta_mpg, 
...
hashtag19 <- hashtag19 %>% 
  ...
  mutate(ht_botb_fpts = ht_pts + (ht_dreb*1.5) + (ht_orb*2) + (ht_ast*2) + (ht_stl*2.5) + (ht_blk*2.5) +
  ...
  mutate(ht_botb_fpts = ht_pts + ht_dreb + (ht_orb*1.25) + (ht_ast*1.5) + (ht_stl*1.5) + (ht_blk*2) +
         (ht_tov*-1)+ht_threes+(ht_fga*-0.5)+(ht_fgm*0.5)+(ht_ftm*0.5)+(ht_fta*-0.75)) %>% 
  select(player, ht_botb_fpts, ht_botb_fpts, ht_botb_fpts, ht_botb_fpts, ht_gms, ht_mpg, 
...
combined_data <- combined_data %>% 
  full_join(botb_contracts, by = "player") %>% select(-botb_team)

# in-season fantrax 

botb_inseason$player <- botb_inseason$Player %>% 
  str_replace_all(c('Á' = 'A', 'ā' = 'a', 'á' = 'a', 'č' = 'c', 'ć' = 'c', 'è' = 'e', 'é' = 'e', 'ê' = 'e',
                    'ö' = 'o', 'ó' = 'o', 'Š' = 'S', 'š' = 's', 'ū' = 'u', 'ý' = 'y', 'Ž' = 'Z', 'ž' = 'z',
                    'P.J.' = 'PJ','C.J.' = 'CJ', 'R.J.' = 'RJ', 'T.J.' = 'TJ', 'D.J.' = 'DJ', 'J.J.' = 'JJ',
                    'Louis' = 'Lou', 'Edrice' = 'Bam', 'Waller-Prince' = 'Prince', "O.G." = "OG", 
...
botb_inseason <- botb_inseason %>% 
  rename(
         #trade_comp = "Trade",
         fntx_gms = "GP", fntx_fppg = "FP/G", botb_team = "Status", trade_comp = "Trade", position = "Position") %>%
  rename(nba_team = "Team") %>% 
  mutate(nba_team = recode(nba_team, PHO = "PHX", SA = "SAS", NY = "NYK", NO = "NOP", GS = "GSW")) %>% 
  select(player, fntx_fppg, fntx_gms, botb_team, nba_team, position, trade_comp
         )
         
select_data <- combined_data %>% 
  select(-trade_comp, -nba_team) %>% 
  inner_join(botb_inseason, by = "player") %>%   
  select(player, age, fntx_fppg, bbr_botb_fpts, hta_botb_fpts, ht_botb_fpts, 
         bbr_gms, bbr_mpg, hta_gms, hta_mpg, ht_gms, ht_mpg,
         botb_team, botb_2019, botb_2020, botb_2021, botb_2022, botb_2023, 
         botb_rfa, botb_gleague, nba_team, position, trade_comp 
         #botb_intl, trade_comp
         ) %>%   
...
rm(bballref18, hashtag19, hashalt19, franchise_contracts, legacy_contracts)
```
...
select_data %>% 
  ...
  #filter(trade_comp == "DAL") %>% 
  #filter(bbr_gms > 50) %>%        # by games played
  #filter(bbr_botb_fpts > 25) %>%    # by 2018 stats
  #filter(hta_botb_fpts > 25) %>%    # by hta projections 
  #filter(ht_botb_fpts > 25) %>%     # by ht projections 
  #filter(age < 25) %>%            # by age 
  #arrange(-bbr_botb_fpts) %>%       # arrange by 2018 stats
  #arrange(-hta_botb_fpts) %>%       # arrange by hta projections
  arrange(-fntx_fppg) %>%         # arrange by ht projections
  select(player, age, fntx_fppg, position, botb_2019, botb_2020, botb_2021, botb_rfa, 
         bbr_botb_fpts, hta_botb_fpts, ht_botb_fpts,  botb_gleague#, botb_intl
         )
```
...
select_data %>% 
  mutate(fntx_value_19 = fntx_fppg / botb_2019 * 10000) %>% 
  mutate(fntx_value_20 = fntx_fppg / botb_2020 * 10000) %>%
  mutate(fntx_value_21 = fntx_fppg / botb_2021 * 10000) %>%
  mutate(diff = fntx_fppg - ht_botb_fpts) %>%
  filter(fntx_fppg > 18) %>% 
  filter(botb_2019 < 9600000) %>% 
  filter(botb_team == "SAC" | botb_team == "MIL" | botb_team == "BOS" | botb_team == "VAN" |  
         botb_team == "ORL" | botb_team == "TOR" | botb_team == "DET" | botb_team == "DAL" | 
         botb_team == "NOP" | botb_team == "CLE" | botb_team == "MIN" | botb_team == "NYK"  
  ) %>%
  #filter(botb_team == "CLE") %>% 
  #filter(age < 25) %>% 
  #filter(grepl("C", position)) %>% 
  arrange(-fntx_value_19) %>% 
  select(player, age, botb_team, fntx_fppg, position, ht_botb_fpts, botb_2019, botb_2020, fntx_value_19,diff,
         fntx_value_20,   botb_2021, botb_rfa, botb_gleague)
```

```{r per-week-scores, fig.width=12, fig.height=10, warning=FALSE}

# regular season 
week1_totals <- select_data %>% group_by(botb_team) %>% top_n(n = 12, wt = fntx_fppg) %>% summarise(week1_totals = sum(week1_totals)) 
week2_totals <- select_data %>% group_by(botb_team) %>% top_n(n = 12, wt = fntx_fppg) %>% summarise(week2_totals = sum(week2_totals)) 
week3_totals <- select_data %>% group_by(botb_team) %>% top_n(n = 12, wt = fntx_fppg) %>% summarise(week3_totals = sum(week3_totals)) 
week4_totals <- select_data %>% group_by(botb_team) %>% top_n(n = 12, wt = fntx_fppg) %>% summarise(week4_totals = sum(week4_totals)) 
week5_totals <- select_data %>% group_by(botb_team) %>% top_n(n = 12, wt = fntx_fppg) %>% summarise(week5_totals = sum(week5_totals)) 
week6_totals <- select_data %>% group_by(botb_team) %>% top_n(n = 12, wt = fntx_fppg) %>% summarise(week6_totals = sum(week6_totals)) 
week7_totals <- select_data %>% group_by(botb_team) %>% top_n(n = 12, wt = fntx_fppg) %>% summarise(week7_totals = sum(week7_totals)) 
week8_totals <- select_data %>% group_by(botb_team) %>% top_n(n = 12, wt = fntx_fppg) %>% summarise(week8_totals = sum(week8_totals)) 
week9_totals <- select_data %>% group_by(botb_team) %>% top_n(n = 12, wt = fntx_fppg) %>% summarise(week9_totals = sum(week9_totals))
week10_totals <- select_data %>% group_by(botb_team) %>% top_n(n = 12, wt = fntx_fppg) %>% summarise(week10totals = sum(week10_totals))
week11_totals <- select_data %>% group_by(botb_team) %>% top_n(n = 12, wt = fntx_fppg) %>% summarise(week11_totals = sum(week11_totals)) 
week12_totals <- select_data %>% group_by(botb_team) %>% top_n(n = 12, wt = fntx_fppg) %>% summarise(week12_totals = sum(week12_totals)) 
week13_totals <- select_data %>% group_by(botb_team) %>% top_n(n = 12, wt = fntx_fppg) %>% summarise(week13_totals = sum(week13_totals)) 
week14_totals <- select_data %>% group_by(botb_team) %>% top_n(n = 12, wt = fntx_fppg) %>% summarise(week14_totals = sum(week14_totals)) 
week15_totals <- select_data %>% group_by(botb_team) %>% top_n(n = 12, wt = fntx_fppg) %>% summarise(week15_totals = sum(week15_totals)) 
week16_totals <- select_data %>% group_by(botb_team) %>% top_n(n = 12, wt = fntx_fppg) %>% summarise(week16_totals = sum(week16_totals)) 
week17_totals <- select_data %>% group_by(botb_team) %>% top_n(n = 12, wt = fntx_fppg) %>% summarise(week17_totals = sum(week17_totals)) 
week18_totals <- select_data %>% group_by(botb_team) %>% top_n(n = 12, wt = fntx_fppg) %>% summarise(week18_totals = sum(week18_totals)) 
#week19_totals <- select_data %>% group_by(botb_team) %>% top_n(n = 15, wt = fntx_fppg) %>% summarise(week19_totals = sum(week19_totals))

# playoff totals by team 
playoff1_totals <- select_data %>% group_by(botb_team) %>% summarise(playoff1_totals = sum(playoff1_totals))
playoff2_totals <- select_data %>% group_by(botb_team) %>% summarise(playoff2_totals = sum(playoff2_totals))
playoff3_totals <- select_data %>% group_by(botb_team) %>% summarise(playoff3_totals = sum(playoff3_totals))
playoff4_totals <- select_data %>% group_by(botb_team) %>% summarise(playoff4_totals = sum(playoff4_totals))

weekly_totals <- right_join(week1_totals,week2_totals, by = "botb_team") %>% 
  right_join(week3_totals, by = "botb_team") %>% 
  right_join(week4_totals, by = "botb_team") %>% 
  right_join(week5_totals, by = "botb_team") %>% 
  right_join(week6_totals, by = "botb_team") %>% 
  right_join(week7_totals, by = "botb_team") %>% 
  right_join(week8_totals, by = "botb_team") %>% 
  right_join(week9_totals, by = "botb_team") %>%
  right_join(week10_totals, by = "botb_team") %>%
  right_join(week11_totals, by = "botb_team") %>%
  right_join(week12_totals, by = "botb_team") %>% 
  right_join(week13_totals, by = "botb_team") %>% 
  right_join(week14_totals, by = "botb_team") %>% 
  right_join(week15_totals, by = "botb_team") %>% 
  right_join(week16_totals, by = "botb_team") %>% 
  right_join(week17_totals, by = "botb_team") %>% 
  right_join(week18_totals, by = "botb_team") %>% 
  right_join(playoff1_totals, by = "botb_team") %>% 
  right_join(playoff2_totals, by = "botb_team") %>% 
  right_join(playoff3_totals, by = "botb_team") %>% 
  right_join(playoff4_totals, by = "botb_team")  
...
temp_1 <- select_data %>% 
  #filter(botb_injured == "0") %>% 
  #filter(botb_gleague == "0") %>% 
  #filter(botb_intl == "0") %>%  
  filter(botb_team != "FA") %>% drop_na(botb_team) %>%
  group_by(botb_team) %>% top_n(12, wt = fntx_fppg) %>% summarize(avg_fppg = mean(fntx_fppg, na.rm = TRUE)) 
temp_2 <- select_data %>% 
  #filter(botb_injured == "0") %>% 
  #filter(botb_gleague == "0") %>% 
  #filter(botb_intl == "0") %>% 
  filter(botb_team != "FA") %>% drop_na(botb_team) %>%
  group_by(botb_team) %>% top_n(12, wt = fntx_fppg) %>% summarize(botb_players = n()) %>% filter(botb_team != "NA") %>% 
...
temp_3 <- select_data %>% 
  ...
  #filter(botb_intl == "0") %>% 
  filter(botb_team != "FA") %>% drop_na(botb_team) %>%
  group_by(botb_team) %>% top_n(12, wt = fntx_fppg) %>% 
  summarize(cap_used = sum(botb_2019, na.rm = TRUE)) %>% 
  mutate(cap_left = 109400000 - cap_used)
pre_trade <- full_join(temp_2, temp_3, by = "botb_team") %>% 
...
temp_2 <- select_data %>% 
  ...
  #filter(botb_intl == "0") %>% 
  filter(botb_team != "FA") %>% drop_na(botb_team) %>%
  group_by(trade_comp) %>% top_n(12, wt = fntx_fppg) %>% summarize(botb_players = n()) %>% filter(trade_comp != "NA") %>% 
...
ggplot(data=pre_trade, 
  ...
  geom_text(aes(label = botb_players), size = 3, colour="white", fontface="bold", hjust = 0.5, vjust = 5) +
...
ggplot(data=pre_trade, aes(x=reorder(botb_team, -avg_fppg), y=avg_fppg, fill=botb_team)) +
  geom_bar(stat="identity", width=0.6, colour="black") +
  theme_bw() + theme(axis.title.x=element_blank(), axis.title.y=element_blank()) +
